...
class Puzzle:
    ...
    def simulateMove(self, dir):
        # Crear una copia manual del tablero
        simPuzzle = Puzzle(self.boardSize, shuffle=False)
        simPuzzle.board = [row[:] for row in self.board]  # Copia profunda de cada fila del tablero
        simPuzzle.blankPos = self.blankPos  # Copia de la posición en blanco

        if not simPuzzle.move(dir):
            print(f"Movimiento simulado no válido: {dir}")
            return False, None
